...
class KegiatanPenunjangController extends Controller
{
    ...
    public function homeBongkarMuat(){
        ...
        $bongkarmuat = KegiatanPenunjangBongkarMuat::where('id_perusahaan' , $user->id)
            ->whereYear('mulai', '=', $time)
            ->whereMonth('mulai', '=', $time)
            ->orderBy('mulai', 'asc')
            ->get();
        $suratMasuk = Surat::where('id_tujuan', $user->id_user)->get();
        $suratKeluar = Surat::where('id_user', $user->id_user)->get();
        return view('layout.kegiatanpenunjang.bongkarmuat.home', compact(
            'user', 'bongkarmuat', 'suratMasuk', 'suratKeluar', 'bm'
        ));
    }
    ...
    public function homePelayaranRakyat(){
        ...
        for ($i = 1; $i <= $bulan; $i++){
            ...
            $tgl = new Carbon($dates);
            $tgl->format('Y-m-d');
            $perla = KegiatanPenunjangPerla::where('id_perusahaan' , $user->id)
        ...
        $perla = KegiatanPenunjangPerla::where('id_perusahaan' , $user->id)
            ->whereYear('tb_tgl', '=', $time)
            ->whereMonth('tb_tgl', '=', $time)
            ->orderBy('tb_tgl', 'asc')
            ->get();
        $suratMasuk = Surat::where('id_tujuan', $user->id_user)->get();
        $suratKeluar = Surat::where('id_user', $user->id_user)->get();
        return view('layout.kegiatanpenunjang.pelayaranrakyat.home', compact(
            'user', 'perla', 'suratMasuk', 'suratKeluar', 'pr'
        ));
    }
    ...
    public function homePengurusanTransportasi(){
        ...
        for ($i = 1; $i <= $bulan; $i++){
            ...
            $tgl = new Carbon($dates);
            $tgl->format('Y-m-d');
            $pengurusan = KegiatanPenunjangPengurusanTransportasi::where('id_perusahaan' , $user->id)
                ->whereYear('tanggal', '=', $tgl)
                ->whereMonth('tanggal', '=', $tgl)
                ->orderBy('tanggal', 'asc')
                ->get();

            array_push( $pt, $pengurusan);
        }

        $pengurusan = KegiatanPenunjangPengurusanTransportasi::where('id_perusahaan' , $user->id)
            ->whereYear('tanggal', '=', $time)
            ->whereMonth('tanggal', '=', $time)
            ->orderBy('tanggal', 'asc')
            ->get();
        $suratMasuk = Surat::where('id_tujuan', $user->id_user)->get();
        $suratKeluar = Surat::where('id_user', $user->id_user)->get();
        return view('layout.kegiatanpenunjang.pengurusantransportasi.home', compact(
            'user', 'pengurusan', 'suratMasuk', 'suratKeluar', 'pt'
        ));
    }
    ...
    // Controller Surat Masuk Bongkar Muat
    public function suratMasukBongkarMuat(){
        $user = $this->getUser();
        if(!Session::has('user')){
            return redirect('login')->with('alert','Kamu harus login dulu');
        }else{
            ...
            $surat_list = DB::table('tb_surat')
                ...
                ->get();
            return view('layout.kegiatanpenunjang.bongkarmuat.suratmasuk', compact('surat_list', 'value', 'user'));
        }
    }

    // Controller Surat Keluar Bongkar Muat
    public function suratKeluarBongkarMuat(){
        $user = $this->getUser();
        if(!Session::has('user')){
            return redirect('login')->with('alert','Kamu harus login dulu');
        }else{
            ...
            $surat_list = DB::table('tb_surat')
                ...
                ->select('tb_surat.*', 'users.email')
                ->get();
            return view('layout.kegiatanpenunjang.bongkarmuat.suratkeluar', compact('surat_list', 'value', 'user', 'tujuan_list'));
        }
    }

    // Tambah Surat Bongkar Muat
    public function tambahSuratBongkarMuat(Request $request){
        ...
        $surat->save();
        return redirect('/kegiatanpenunjang/bongkarmuat/suratKeluar')->with(['succes' => 'Berhasil mengirim surat!']);
    }

    // Hapus Surat Bongkar Muat
    public function hapusSuratBongkarMuat($id){
        $surat = Surat::FindOrFail($id);
        File::delete('file_surat/'.$surat->file);
        $surat->delete();
        return redirect('/kegiatanpenunjang/bongkarmuat/suratKeluar')->with(['succes' => 'Berhasil Hapus Surat!']);
    }

    // Controller Surat Masuk Perla
    public function suratMasukPerla(){
        $user = $this->getUser();
        if(!Session::has('user')){
            return redirect('login')->with('alert','Kamu harus login dulu');
        }else{
            ...
            $surat_list = DB::table('tb_surat')
                ...
                ->select('tb_surat.*', 'users.email')
                ->get();
            return view('layout.kegiatanpenunjang.pelayaranrakyat.suratmasuk', compact('surat_list', 'value', 'user'));
        }
    }

    // Controller Surat Keluar Perla
    public function suratKeluarPerla(){
        $user = $this->getUser();
        if(!Session::has('user')){
            return redirect('login')->with('alert','Kamu harus login dulu');
        }else{
            ...
            $surat_list = DB::table('tb_surat')
                ...
                ->select('tb_surat.*', 'users.email')
                ->get();
            return view('layout.kegiatanpenunjang.pelayaranrakyat.suratkeluar', compact('surat_list', 'value', 'user', 'tujuan_list'));
        }
    }

    // Tambah Surat Perla
    public function tambahSuratPerla(Request $request){
        ...
        $surat->file = Carbon::now()->format('d-m-Y-H-i-s').$file->getClientOriginalName();
        $surat->save();
        return redirect('/kegiatanpenunjang/pelayaranrakyat/suratKeluar')->with(['succes' => 'Berhasil mengirim surat!']);
    }

    // Hapus Surat Perla
    public function hapusSuratPerla($id){
        $surat = Surat::FindOrFail($id);
        File::delete('file_surat/'.$surat->file);
        $surat->delete();
        return redirect('/kegiatanpenunjang/pelayaranrakyat/suratKeluar')->with(['succes' => 'Berhasil Hapus Surat!']);
    }

    // Controller Surat Masuk Pengurusan Transportasi
    public function suratMasukPengurusanTransportasi(){
        $user = $this->getUser();
        if(!Session::has('user')){
            return redirect('login')->with('alert','Kamu harus login dulu');
        }else{
            ...
            $surat_list = DB::table('tb_surat')
                ...
                ->select('tb_surat.*', 'users.email')
                ->get();
            return view('layout.kegiatanpenunjang.pengurusantransportasi.suratmasuk', compact('surat_list', 'value', 'user'));
        }
    }

    // Controller Surat Keluar Pengurusan Transportasi
    public function suratKeluarPengurusanTransportasi(){
        $user = $this->getUser();
        if(!Session::has('user')){
            return redirect('login')->with('alert','Kamu harus login dulu');
        }else{
            ...
            $surat_list = DB::table('tb_surat')
                ...
                ->select('tb_surat.*', 'users.email')
                ->get();
            return view('layout.kegiatanpenunjang.pengurusantransportasi.suratkeluar', compact('surat_list', 'value', 'user', 'tujuan_list'));
        }
    }

    // Tambah Surat Pengurusan Transportasi
    public function tambahSuratPengurusanTransportasi(Request $request){
        ...
        $surat->file = Carbon::now()->format('d-m-Y-H-i-s').$file->getClientOriginalName();
        $surat->save();
        return redirect('/kegiatanpenunjang/pengurusantransportasi/suratKeluar')->with(['succes' => 'Berhasil mengirim surat!']);
    }

    // Hapus Surat Pengurusan Transportasi
    public function hapusSuratPengurusanTransportasi($id){
        $surat = Surat::FindOrFail($id);
        File::delete('file_surat/'.$surat->file);
        $surat->delete();
        return redirect('/kegiatanpenunjang/pengurusantransportasi/suratKeluar')->with(['succes' => 'Berhasil Hapus Surat!']);
    }
}
